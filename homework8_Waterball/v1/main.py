...
# 當直接以 `python main.py` 作為腳本主程序執行時，Python 預設 __package__ 為 None。
# 為了讓 v1 套件內部的相對 import (如 ..common) 能在任意目錄下自動生效，
# 我們動態取得所在套件名稱並設定 __package__，完全解除對硬編碼目錄名稱的依賴。
if __name__ == "__main__" and __package__ is None:
    v1_dir = Path(__file__).resolve().parent
    v1_parent = v1_dir.parent
    if str(v1_parent) not in sys.path:
        sys.path.insert(0, str(v1_parent))
    __package__ = v1_dir.name

from .bot import BotFacade
