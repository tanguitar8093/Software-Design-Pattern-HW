...
class FsmPlugin(ABC):
    ...
    def onPreFire(self, fsm: FiniteStateMachine, context: TransitionContext) -> bool:
        """
        在 FSM 評估常態轉移 (Transitions) 之前觸發。
        若回傳 True，表示插件已成功消化該事件（如由子狀態機內部處理），FSM 不再執行外層轉移。
        """
        pass

    def onPostChangeState(self, fsm: FiniteStateMachine, oldState: Optional[Any], newState: Optional[Any], context: TransitionContext) -> None:
        """狀態切換後的掛鉤回呼 (可選實作)"""
        pass
