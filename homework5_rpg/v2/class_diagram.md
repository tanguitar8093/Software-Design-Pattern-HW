...
為了徹底消滅 `v1` 中違反 OCP 的 `if-else` 與 `switch-case`，這次重構將會套用以下 5 個設計模式：
...
5. **與狀態結合的雙重派發 (Double Dispatch / Visitor Pattern)**: 解決「一拳攻擊 (OnePunch)」強制耦合目標狀態的問題。`OnePunch` 不自己判斷，而是呼叫目標目前 `State` 的 `take_one_punch_damage()` 讓狀態多型決定受傷邏輯。
...
classDiagram
    ...
    class State {
        ...
        +take_one_punch_damage(target_unit)
    }
    class NormalState
    class PoisonedState
    ...
    %% 雙重派發關係
    OnePunch ..> State : Double Dispatch
```
