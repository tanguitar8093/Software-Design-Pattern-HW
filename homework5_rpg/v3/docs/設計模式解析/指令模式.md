...
- `BattleEngine` 想維持統一的戰鬥流程，不想知道每一招技能各自怎麼實作。
...
```mermaid
flowchart TD
    A[BattleEngine.run_battle] --> B{選到哪個技能?}
